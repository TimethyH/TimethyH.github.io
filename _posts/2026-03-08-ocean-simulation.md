...
In code, `Cosine2s` is this function and `NormalizationFactor` is $Q(s)$, computed using a polynomial approximation rather than the exact Euler gamma function for performance reasons.
...
Where $\xi$ is the `swell` parameter. Increasing swell adds to $s$, making waves more elongated and parallel, simulating waves that have travelled from a distant storm rather than being generated by local wind.
